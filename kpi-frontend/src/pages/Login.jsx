...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      alert("Login ล้มเหลว: " + (err.response?.data?.msg || "ไม่สามารถเข้าสู่ระบบได้"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800">
        ...
        {mounted && (
          <>
            <FloatingParticle delay={0} duration={6} size={2} left={10} top={20} />
            <FloatingParticle delay={1} duration={8} size={1} left={80} top={10} />
            <FloatingParticle delay={2} duration={7} size={3} left={60} top={70} />
            <FloatingParticle delay={3} duration={9} size={2} left={20} top={80} />
            <FloatingParticle delay={4} duration={5} size={1} left={90} top={60} />
            <FloatingParticle delay={5} duration={10} size={2} left={40} top={30} />
          </>
        )}

        {/* Geometric shapes */}
        <div className="absolute top-20 left-20 w-32 h-32 border border-white/20 rotate-45 animate-spin opacity-30" style={{ animationDuration: '20s' }} />
        <div className="absolute bottom-40 right-20 w-24 h-24 border-2 border-white/10 rounded-full animate-bounce opacity-40" style={{ animationDuration: '3s' }} />
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4">
        ...
        `}>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center text-gray-300 cursor-pointer group">
                <input type="checkbox" className="w-4 h-4 text-cyan-400 bg-white/10 border-white/20 rounded focus:ring-cyan-400 focus:ring-2 mr-2" />
          ...
          <p className="text-center text-gray-300">
            ...
            <a href="/register" className="text-cyan-400 hover:text-cyan-300 font-semibold transition-colors duration-200 hover:underline">
              Create one
            </a>
          </p>
        </div>
